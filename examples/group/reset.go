...
func main() {
	...
	reqId, err := client.ResetHardwareGroup(flag.Arg(0), *acctAlias)
	if err != nil {
		exit.Fatalf("Failed to reset hardware group: %s", err)
	}

	fmt.Println("Request ID for resetting hardware group:", reqId)
}
