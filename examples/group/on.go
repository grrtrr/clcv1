...
func main() {
	...
	reqId, err := client.PowerOnHardwareGroup(flag.Arg(0), *acctAlias)
	if err != nil {
		exit.Fatalf("Failed to power on hardware group: %s", err)
	}

	fmt.Println("Request ID for powering on hardware group:", reqId)
}
