...
func main() {
	...
	reqId, err := client.PowerOffHardwareGroup(flag.Arg(0), *acctAlias)
	if err != nil {
		exit.Fatalf("Failed to power off hardware group: %s", err)
	}

	fmt.Println("Request ID for powering hardware group off:", reqId)
}
