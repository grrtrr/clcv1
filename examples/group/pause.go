...
func main() {
	...
	reqId, err := client.PauseHardwareGroup(flag.Arg(0), *acctAlias)
	if err != nil {
		exit.Fatalf("Failed to pause hardware group: %s", err)
	}

	fmt.Println("Request ID for pausing hardware group:", reqId)
}
