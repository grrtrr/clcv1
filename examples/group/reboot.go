...
func main() {
	...
	reqId, err := client.RebootHardwareGroup(flag.Arg(0), *acctAlias)
	if err != nil {
		exit.Fatalf("Failed to reboot hardware group: %s", err)
	}

	fmt.Println("Request ID for rebooting hardware group:", reqId)
}
